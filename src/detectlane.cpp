...
DetectLane::DetectLane()
{
    ...
    cvCreateTrackbar("line", "Threshold", &Line, 140);
    cvCreateTrackbar("skyline", "Threshold", &skyLine, 255);
...
void DetectLane::update(const Mat &src)
{
    ...
    while (leftLane[i] == DetectLane::null && rightLane[i] == DetectLane::null)
        ...
        circle(lane, pos, 1, (0, 0, 255), LINE_8, 0);
    ...
    // imwrite("lane" + to_string(C) + ".png", lane);
    C ++;
...
Mat DetectLane::preProcess(const Mat &src)
{
    ...
    // string type = type2str(imgThresholded.type());
    // cout << endl << type;

    for (int i = 0; i < 3*imgThresholded.rows / 8; i++)
    {
    ...
    // {
    //     Rect boundRect_temp = boundingRect(contours[index]);
    //     double max = 0;
    //     vector<Point> approx;

    //     approxPolyDP(contours[index], approx, 5, true);
        
    //     double area = contourArea(approx);
        
        
    //     if (  area > max && boundRect_temp.width/boundRect_temp.height < 3  && boundRect_temp.width/boundRect_temp.height > 1/3)
    //     {
            
    //         cout << "a" << endl;
    //         max = area;
    //         indexMax = index;
    //         bound = boundingRect(approx);
    //     }
    // }
    ...
    // findContours(imgThresholded, cnt, CV_RETR_EXTERNAL, CV_CHAIN_APPROX_SIMPLE);
    // Mat contourIMG(imgHSV.size(), CV_8UC1);
    // drawContours(contourIMG, cnt, 1, Scalar(255));
    // imshow("contour", contourIMG);
    morphological(imgThresholded);
    dst = birdViewTranform(imgThresholded);

    fillLane(dst);
    imshow("Bird View", dst);

    // rectangle(imgThresholded, bound, 255); 
    // imgThresholded(bound).setTo(cv::Scalar(0));
...
void DetectLane::detectLeftRight(const vector<vector<Point>> &points)
{
    ...
    // line(test, Point(line1[2], line1[3]), Point(lane1X, BIRDVIEW_HEIGHT), Scalar(255, 0, 0));
    // line(test, Point(line2[2], line2[3]), Point(lane2X, BIRDVIEW_HEIGHT), Scalar(0, 255, 0));
    // circle(test, Point(lane1X, BIRDVIEW_HEIGHT), 2, Scalar(255, 255, 0));
    // circle(test, Point(lane2X, BIRDVIEW_HEIGHT), 2, Scalar(0, 255, 255));
    // circle(test, Point(120, BIRDVIEW_HEIGHT), 2, Scalar(255, 0, 255));
    // imshow("test", test);
...
Mat DetectLane::birdViewTranform(const Mat &src)
{
    ...
    dst_vertices[2] = Point(BIRDVIEW_WIDTH - Line, BIRDVIEW_HEIGHT);
    dst_vertices[3] = Point(Line, BIRDVIEW_HEIGHT);

    Mat M = getPerspectiveTransform(src_vertices, dst_vertices);
    //imshow("perspective", M);
